...
def _ensure_active_device(device: BackendDevice | None) -> BackendDevice:
...
class NDArray:
    ...
    def make(
        ...
        array._device = _ensure_active_device(device)
        if handle is None:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        # Check if the array is compact
        if not self.is_compact():
            raise ValueError("Array must be compact to reshape")
        
        # Check if the total number of elements matches
        if prod(self._shape) != prod(new_shape):
            raise ValueError(f"Cannot reshape array of size {prod(self._shape)} into shape {new_shape}")
        
        # Create new NDArray with the same memory but different shape
        return NDArray.make(
            shape=new_shape,
    ...
    def permute(self, new_axes: tuple[int, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        # Validate that new_axes is a valid permutation
        if len(new_axes) != len(self._shape):
            raise ValueError(f"Permutation axes length {len(new_axes)} must match array dimensions {len(self._shape)}")
        
        if set(new_axes) != set(range(len(self._shape))):
            raise ValueError(f"Permutation axes {new_axes} must be a valid permutation of {list(range(len(self._shape)))}")
        
        # Create new shape and strides by reordering according to new_axes
        new_shape = tuple(self._shape[i] for i in new_axes)
        new_strides = tuple(self._strides[i] for i in new_axes)
        
        # Create new NDArray with permuted dimensions
        return NDArray.make(
            shape=new_shape,
            strides=new_strides,
            device=self._device,
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        if len(new_shape) != len(self._shape):
          raise ValueError("Error in shape size")
        
        new_strides = []
        for i in range(len(new_shape)):
            if self._shape[i] == 1:
                new_strides.append(0)  # Broadcast dimension: stride = 0
            else:
                new_strides.append(self._strides[i])  # Keep original stride
        return NDArray.make(
          shape=new_shape,
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        ### BEGIN YOUR SOLUTION
        # Calculate new shape, strides, and offset based on slices
        new_shape = []
        new_strides = []
        new_offset = self._offset
        
        for i, sl in enumerate(slices):
            # Calculate the size of this dimension after slicing
            dim_size = (sl.stop - sl.start + sl.step - 1) // sl.step
            new_shape.append(dim_size)
            
            # Calculate new stride: original stride * step
            new_strides.append(self._strides[i] * sl.step)
            
            # Update offset: add start * original stride
            new_offset += sl.start * self._strides[i]
        
        # Create new NDArray with sliced dimensions
        return NDArray.make(
    ...
    def flip(self, axes: tuple[int, ...]) -> "NDArray":
        ...
        for axis in axes:
            if axis < 0 or axis >= len(self._shape):
                raise ValueError(f"Axis {axis} out of range for array with {len(self._shape)} dimensions")
        
        # Convert axes to a set for efficient lookup
        axes_set = set(axes)
        
        # Calculate new strides: negate strides for flipped axes
        new_strides = tuple(
            -self._strides[i] if i in axes_set else self._strides[i]
            for i in range(len(self._shape))
        )
        
        # Calculate new offset: for each flipped axis, we need to start at position
        # (shape[i] - 1) * original_strides[i]
        new_offset = self._offset
        for axis in axes_set:
            new_offset += (self._shape[axis] - 1) * self._strides[axis]
        
        # Create a new view with negative strides and adjusted offset
        ...
        # Compact to get positive strides (as per instructions)
    ...
    def pad(self, axes: tuple[tuple[int, int], ...]) -> "NDArray":
        ...
        """
        ### BEGIN YOUR SOLUTION
        # Validate that axes has the correct length
        if len(axes) != len(self._shape):
            raise ValueError(f"axes length {len(axes)} must match array dimensions {len(self._shape)}")
        
        # Calculate the new shape after padding
        new_shape = tuple(
            self._shape[i] + axes[i][0] + axes[i][1]
            for i in range(len(self._shape))
        )
        ...
        # Create slices to specify where to place the original array
        # For each dimension i: slice from axes[i][0] to axes[i][0] + original_shape[i]
        slices = tuple(
            slice(axes[i][0], axes[i][0] + self._shape[i])
            for i in range(len(self._shape))
        )
        
        # Copy the original array into the padded array at the correct position
        padded[tuple(slices)] = self.compact()
